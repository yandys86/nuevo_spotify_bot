...
echo "================================================"
echo "   YiyoLMB - Despliegue Total v3.0 (qm)"
echo "================================================"
...
for VM_ID in $VMS; do
    ...
    # 2. Sincronizar cuenta desde la 111
    ...
    # 3. Permisos de pantalla y arranque del Bot
    ...
    # 4. Forzar Play con Python corregido (import time)
    echo "[VM $VM_ID] Forzando inicio de música..."
    sleep 5
    qm guest exec $VM_ID -- /bin/bash -c "sudo -u localuser bash -c 'export DISPLAY=:0; export XAUTHORITY=/home/localuser/.Xauthority; cd $BOT_DIR; ./venv/bin/python3 -c \"import pyautogui; import time; time.sleep(10); pyautogui.press(\\\"esc\\\"); time.sleep(1); pyautogui.click(600, 400); time.sleep(1); pyautogui.press(\\\"space\\\")\"'"
